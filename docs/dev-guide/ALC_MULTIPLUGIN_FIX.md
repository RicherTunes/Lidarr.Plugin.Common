...
- **C3 — packaging-policy split**: `tools/PluginPack.psm1` (and a few validators) still inject + SHA-pin a sidecar `Lidarr.Plugin.Abstractions.dll`, which contradicts `PluginPackaging.targets`'s deletion of it. `scripts/lib/e2e-abstractions.psm1` is updated in this PR to tolerate the merged-DLL case (no longer throws when no sidecar found). **Tracked follow-up**: audit `PluginPack.psm1`, `PackageClosureValidator.DefaultAllowedAssemblies`, and `PluginPackageValidator` to remove the sidecar-required path entirely.
