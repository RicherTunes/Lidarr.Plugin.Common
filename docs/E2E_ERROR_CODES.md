...
| `E2E_ABSTRACTIONS_SHA_MISMATCH` | Legacy sidecar `Lidarr.Plugin.Abstractions.dll` copies have non-identical bytes. | One or more plugins still ship sidecars from different Common commits. | Rebuild from the same Common SHA or, preferably, rebuild with merged/internalized Abstractions and no sidecar. |
