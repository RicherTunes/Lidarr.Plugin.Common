...
| 35 | Plugin-state recovery on restart (download-queue checkpointing) | N/A for download checkpoint — `FileUnresolvedQueueStore` persists IMPORT-LIST unresolved-album candidates (different concern) | ✓ terminal-state checkpoint via Common `HostBridgeDownloadTrackerStore.ForPlugin`; ⚠ queued/downloading entries intentionally dropped on restart until a resumable worker seam exists | ✓ terminal-state checkpoint via Common `HostBridgeDownloadTrackerStore.ForPlugin`; ⚠ queued/downloading entries intentionally dropped on restart until a resumable worker seam exists | N/A — import-list, no download queue |
