...
public static class LlmErrorMapper
{
    ...
    public static TimeSpan? ParseRetryAfterHeader(RetryConditionHeaderValue? header)
    {
        ...
        // Keep absence distinct so response-body hints can still be used. A present
        // header, including an expired date or zero delta, overrides those hints.
        return RateLimitHeaderUtilities.ResolveRetryAfter(header);
    }
