        ...
        public string SanitizeFileName(string fileName)
        {
            try
            {
                ...
                if (sanitized.Length > MAX_FILENAME_LENGTH)
                {
                    ...
                    _logger.LogDebug("📏 FILENAME TRUNCATED: Original too long, truncated to {0} chars", sanitized.Length);
                }
            ...
            {
                _logger.LogError(ex, "🛡️ DEFENSIVE: Filename sanitization failed for '{0}', using fallback", fileName);
