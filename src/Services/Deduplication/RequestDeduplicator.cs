    ...
    public class RequestDeduplicator
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (_pendingRequests.TryGetValue(key, out var existingTaskInfo))
            {
                if (!existingTaskInfo.IsExpired(_requestTimeout))
                {
                    ...
                    {
                        _logger.LogDebug("⏰ COALESCED REQUEST TIMEOUT: Key '{0}' timed out, falling back to new request", key);
                ...
                {
                    _logger.LogDebug("⏰ EXPIRED REQUEST: Removing expired request for key: {0}", key);
            ...
            DateTime startTime)
        {
            ...
            if (addedTaskInfo != taskInfo)
            {
                ...
                {
                    _logger.LogDebug("❌ CONCURRENT REQUEST FAILED: Fallback execution for key '{0}': {1}", key, ex.Message);
            ...
            _logger.LogDebug("🚀 NEW REQUEST: Executing factory for key: {0}", key);
            ...
            finally
            {
                ...
                _pendingRequests.TryRemove(key, out _);
                _logger.LogDebug("🧹 CLEANUP: Removed pending request for key: {0}", key);
            }
        }
        ...
        private void CleanupExpiredRequests(object? state)
        {
            try
            {
                ...
                {
                    _logger.LogDebug("📊 DEDUPLICATION STATS: {0} active deduplicated requests", _pendingRequests.Count);
                }
            }
            catch (Exception ex)
            {
