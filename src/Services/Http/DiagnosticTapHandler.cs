...
{
    /// <summary>
    /// Opt-in HTTP diagnostics tap. Enable with LIDARR_PLUGIN_HTTP_TAP=1/true/yes.
    /// Logs request/response lines + headers (redacts sensitive values) and mirrors small textual bodies.
    /// </summary>
    public sealed class DiagnosticTapHandler : DelegatingHandler
    {
        ...
        {
            if (headers is not HttpHeaders hh) return;
        ...
        private static string SanitizeUri(Uri? uri)
        {
            ...
            try
            {
                ...
                {
                    var idx = kv.IndexOf('=');
                    var key = idx >= 0 ? kv.Substring(0, idx) : kv;
                    var val = idx >= 0 ? kv.Substring(idx + 1) : string.Empty;
        ...
        private static bool IsTextual(HttpContentHeaders? headers)
        {
            ...
            return mt.StartsWith("text/", StringComparison.OrdinalIgnoreCase)
                   || mt.Equals("application/json", StringComparison.OrdinalIgnoreCase)
                   || mt.Equals("application/problem+json", StringComparison.OrdinalIgnoreCase);
        }
