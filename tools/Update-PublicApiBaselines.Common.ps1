...
$project = Join-Path $PSScriptRoot '..\src\Lidarr.Plugin.Common.csproj'
...
foreach ($tfm in $Tfms) {
  ...
  $dir = Join-Path $PSScriptRoot "..\src\PublicAPI\$tfm"
  $unshipped = Join-Path $dir 'PublicAPI.Unshipped.txt'
  $shipped   = Join-Path $dir 'PublicAPI.Shipped.txt'

  if (-not (Test-Path $shipped)) { throw "Missing shipped baseline for ${tfm} at $shipped" }
  if (-not (Test-Path $unshipped)) { Set-Content -Path $unshipped -Value "#nullable enable`r`n" -Encoding UTF8 }
  ...
  else { Write-Host " ${tfm}: no unshipped entries" }
}
