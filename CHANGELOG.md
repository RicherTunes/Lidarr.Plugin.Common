...
- **`SimpleDownloadOrchestrator` propagates cancellation from post-processing and metadata hooks.** `IAudioPostProcessor` and `IAudioMetadataApplier` extension points can surface `OperationCanceledException`; the catch-all warning paths now rethrow cancellation so callers and album-level orchestration observe cancellation instead of a false successful track.
