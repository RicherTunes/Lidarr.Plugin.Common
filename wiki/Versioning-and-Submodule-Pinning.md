...
Plugin repos consume Common as a git submodule under `ext/`. Alongside the submodule pointer, each plugin keeps an `ext-common-sha.txt` sentinel file whose content must match the submodule's checked-out SHA. GitHub and Gitea CI both run the verify-only repin guard and fail if the gitlink, sentinel, or checked-out submodule state diverge.
...
Plugin repos should include:

- `.github/workflows/submodule-pin.yml` for the focused GitHub pin check.
- `Common submodule pin guard` in `.github/workflows/ci.yml`.
- `Common submodule pin guard` in `.gitea/workflows/ci.yml`.
